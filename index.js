...
var JUnitReporter = function(baseReporterDecorator, config, logger,
    helper, formatError) {
    ...
    if (!reporterConfig.outputDir) {
        ...
        throw new Error('You must set an output directory for ' +
            'JUnitReporter via the outputDir config property');

    }

    var outputDir = helper.normalizeWinPath(
        path.resolve(config.basePath, reporterConfig.outputDir)) + path.sep;
    var suites;
    ...
    var initializeXmlForBrowser = function(browser) {
        ...
        suite.ele('properties')
                .ele('property', {
                    name: 'browser.fullName', value: browser.fullName
                });
    ...
    var writeXmlForBrowser = function(browser) {
        ...
        helper.mkdirIfNotExists(outputDir, function() {
            ...
            fs.writeFile(outputFile, xmlToOutput.end({
                pretty: true
            }), function(err) {
